...
class DemLayer {
    ...
    _setLight() {
        ...
        gl.uniform3fv(this._drawFboModel.u_LightDirection, new Float32Array(normallightDirection));
        //设置环境光
    ...
    onAdd(map, gl) {
        ...
        out vec4 outColor;
        void main() {
            // 从webgl -1--1 坐标转换到 纹理的 0-1坐标
            vec2 v_tetPosition = v_position/2.0+0.5;
            outColor = texture(u_sampler, v_tetPosition);
        }`;
        // fbo
        ...
        // stpe3 构造画布绘制的vao，仅需构造下屏幕纹理坐标即可
        this._drawInfo.vao = gl.createVertexArray();
        gl.bindVertexArray(this._drawInfo.vao);
        const quadBuffer = createBuffer(gl, gl.ARRAY_BUFFER, new Float32Array([
            -1, 1,
            -1, -1,
            1, -1,
            -1, 1,
        ...
        bindAttribute(gl, quadBuffer, 0, 2);
        gl.bindBuffer(gl.ARRAY_BUFFER, null);
        gl.bindVertexArray(null);
