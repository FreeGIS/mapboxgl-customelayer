...
import { createModel, createBuffer, bindAttribute, createIndicesBuffer, bindTexture2D } from '../util/webgl_util';
...
class DemLayer {
    ...
    onAdd(map, gl) {
        ...
        this._drawFboModel = createModel(gl, frame_vs, frame_fs);
        this._drawModel = createModel(gl, vs, fs);
        this._drawInfo = {}, this._drawFboInfo = {};
        // stpe1：创建fbo的vao
        this._drawFboInfo.vao = gl.createVertexArray();
        gl.bindVertexArray(this._drawFboInfo.vao);
        // 绑定坐标顶点
        const fboPositionBuffer = createBuffer(gl, gl.ARRAY_BUFFER, new Float32Array(this._demData.pos));
        bindAttribute(gl, fboPositionBuffer, 0, 3);
        // 绑定颜色顶点
        const fboColorBuffer = createBuffer(gl, gl.ARRAY_BUFFER, new Float32Array(this._demData.color));
        bindAttribute(gl, fboColorBuffer, 1, 3);

        // 顶点索引，unit8array对应gl.UNSIGNED_BYTE
        this._drawFboInfo.elementType = createIndicesBuffer(gl, this._demData.indices, this._demData.pos.length / 3);
        this._drawFboInfo.positionCount = this._demData.indices.length;
        ...
        // 创建纹理对象并设置其尺寸和参数
        this.screenTexture = gl.createTexture(); // 创建纹理对象
        gl.bindTexture(gl.TEXTURE_2D, this.screenTexture); // Bind the object to target
        ...
        // 创建渲染缓冲区对象并设置其尺寸和参数
        const depthBuffer = gl.createRenderbuffer(); //创建渲染缓冲区
        gl.bindRenderbuffer(gl.RENDERBUFFER, depthBuffer); // Bind the object to target
        gl.renderbufferStorage(gl.RENDERBUFFER, gl.DEPTH_COMPONENT16, gl.drawingBufferWidth, gl.drawingBufferHeight);

        // 将纹理和渲染缓冲区对象关联到帧缓冲区对象上
        gl.bindFramebuffer(gl.FRAMEBUFFER, this._framebuffer);
        gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, this.screenTexture, 0);   //关联颜色
        // 关联深度 不关联深度，就等于和不开启深度测试的效果是一样的，只有颜色，没有了深度，在3D绘制中效果会失真。
    ...
    prerender(gl, matrix) {
        // stpe1 绘制到fbo
        gl.bindFramebuffer(gl.FRAMEBUFFER, this._frameBuffer);
        gl.useProgram(this._drawFboModel.program);
        //设置unifrom
        gl.uniformMatrix4fv(this._drawFboModel.u_worldViewProjection, false, matrix);
        //绑定顶点vao
        gl.bindVertexArray(this._drawFboInfo.vao);
        gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT); // Clear FBO
        gl.drawElements(gl.TRIANGLES, this._drawFboInfo.positionCount, this._drawFboInfo.elementType, 0);
    ...
    render(gl, matrix) {
        // stpe2 绘制到屏幕
        gl.useProgram(this._drawModel.program);
        bindTexture2D(gl, this.screenTexture, 13);
    ...
    onRemove(map, gl) {
        gl.deleteProgram(this._drawFboModel.program);
